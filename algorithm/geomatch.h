...
Mat pyrdown(int n, Mat img)
{
  ...
  for(int i=0;i<n;i++)pyrDown(imgs[i], imgs[i+1], Size(imgs[i].cols/2, imgs[i].rows/2));
...
int func(Mat template_img, Mat search_img, int temp_canny_low, int temp_canny_high, int search_canny_low, int search_canny_high) 
{
    ...
    Mat dimg = pyrdown(pyrdown_num, origin);
    ...
    for (int j = 0; j < dimg.rows; j++)
    {
        ...
        {
            if(src[i] == 255){
              circle(dimg, Point(i, j), 1, 0, -1, LINE_AA);
              src[i] = 255;
    ...
    Mat dsimg = pyrdown(pyrdown_num, simg);
